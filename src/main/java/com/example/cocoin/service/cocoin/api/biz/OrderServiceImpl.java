...
public class OrderServiceImpl implements OrderService {
    ...
    private void orderBuy(InsOrderDTO insOrderDTO, UserEntity userEntity) {
        ...
        byUserEntityAndOrderSlctAndCoinSlctAndMarginSlct.stream()
                .findAny()
                .ifPresent(
                        v -> {
                            if ((int) v.getMargin() != insOrderDTO.getMargin()) {
        ...
        byUserEntityAndOrderSlctAndCoinSlctAndMarginSlct.stream()
                .map(v -> BigDecimal.valueOf(v.getCnt()).multiply(BigDecimal.valueOf(v.getPrice())))
                .reduce(BigDecimal::add)
                .ifPresent(
                        v -> {
                            if (pointDec.subtract(v).compareTo(fullPriceDec) < 0) {
                                throw new GeneralException(Code.NOT_ENOUGH_POINT);
                            }
                        }
                );
...
//        if ((int) insMarketDTO.getMargin() != marketEntity.getMargin()) {
//            throw new GeneralException(Code.BAD_REQUEST);
//        }
//
